...
## Primary outcome: first hard flare
...
## Secondary outcome: first soft flare

 /Volumes/igmm/cvallejo-predicct/predicct/old/calpro/20210126/calprotectin.xlsx
 
```{R}
monthlyQ <- as.data.frame(
  read_xlsx(paste0("/Volumes/igmm/cvallejo-predicct/predicct/final/20221004/",
                   "Followup/monthlyQ.xlsx")))
monthlyQ$DiseaseWorsenedDate <- as.Date(monthlyQ$DiseaseWorsenedDate)
monthlyQ$ActualDate <- as.Date(monthlyQ$ActualDate)
monthlyQ <- monthlyQ[!is.na(monthlyQ$DiseaseControlled),]
```

`r nrow(subset(monthlyQ,  ActualDate - DiseaseWorsenedDate > 6 * 30))` subjects
reported a flare more than 6 months after the flare was believed to occur. 
...
```{R}
soft.flare <- data.frame(ParticipantNo = character(),
                         Cens = numeric(),
                         Date = character())

for (id in unique(monthlyQ$ParticipantNo)) {
  sub <- subset(monthlyQ, ParticipantNo == id)
  sub <- sub[order(sub$DiseaseWorsenedDate),]
  if (any(sub$DiseaseControlled == 2)) {
    index <- which(sub$DiseaseControlled == 2)[1]
    Cens <- 1
    Date <- sub[index, "DiseaseWorsenedDate"]
  } else {
    Cens <- 0
    sub <- sub[order(sub$ActualDate), ]
    Date <- sub[nrow(sub), "ActualDate"]
  }
  soft.flare <- rbind(soft.flare,
                      data.frame(ParticipantNo = id, Cens = Cens, Date = Date))
}


soft.flare <- subset(soft.flare, ParticipantNo %in% flare.cut.df$ParticipantNo)
soft.flare <- merge(soft.flare,
                    flare.cut.df[,c("ParticipantNo", "entry_date")],
                    all.x = TRUE,
                    all.y = FALSE)
soft.flare <- soft.flare[!is.na(soft.flare$Date), ]
soft.flare$time <- with(soft.flare, Date - as.Date(entry_date))
soft.flare <- soft.flare[, -4]

hard.flare <- flare.cut.df[, c("ParticipantNo", "DiseaseFlareYN", "FirstFlareStartDate", "time")]
colnames(hard.flare) <- c("ParticipantNo", "Cens", "Date", "time")

soft.flare$type <- "soft"
hard.flare$type <- "hard"

flare.comb <- rbind(soft.flare, hard.flare)
flare.comb$type <- factor(flare.comb$type,
                          levels = c("hard", "soft"),
                          labels = c("hard flare", "soft flare"))

# Censor at two years 
for (i in 1:nrow(flare.comb)){
  if (flare.comb[i, "time"] > 365.25 * 2) {
    flare.comb[i, "Cens"] <- 0
    flare.comb[i, "time"] <- 365.25 * 2
  }
}
```
...
soft.flare <- subset(soft.flare, ParticipantNo %in% flare.df$ParticipantNo)
soft.flare <- merge(soft.flare,
                    flare.cut.df[,c("ParticipantNo", "entry_date")],
                    all.x = TRUE,
                    all.y = FALSE)
...
hard.flare <- flare.df[, c("ParticipantNo", "DiseaseFlareYN", "FirstFlareStartDate", "time")]
colnames(hard.flare) <- c("ParticipantNo", "Cens", "Date", "time")

soft.flare$type <- "soft"
hard.flare$type <- "hard"

soft.flare <- soft.flare %>%
  distinct(ParticipantNo, .keep_all = TRUE)

hard.flare <- hard.flare %>%
  distinct(ParticipantNo, .keep_all = TRUE)

flare.comb <- rbind(soft.flare, hard.flare)
flare.comb[is.na(flare.comb[,"Cens"]), "Cens"] <- 0 
...
for (id in unique(flare.comb$ParticipantNo)) {
  ...
  if (nrow(temp) == 2) {
    if (subset(temp, type == "hard")$Cens == 1) {
      if (subset(temp, type == "soft")$Cens == 0) {
        update <- TRUE
      } else if (subset(temp, type == "soft")$Cens == 1) {
          if (subset(temp, type == "soft")$time > subset(temp, type == "hard")$time) {
          update <- TRUE
          }
        }
    }
  ...
  if (update) {
    flare.comb[flare.comb[,"ParticipantNo"] == id & flare.comb[,"type"] == "soft",
               "Cens"] <- 1
    flare.comb[flare.comb[,"ParticipantNo"] == id & flare.comb[,"type"] == "soft",
               "time"] <- flare.comb[flare.comb[,"ParticipantNo"] == id & flare.comb[,"type"] == "hard", "time"]
  }
}
...
pdf("meat-cd.pdf", width = 8 * 2/3, height = 4.5 * 2/3)
p <-ggsurvplot(fit,
           conf.int = TRUE,
           legend.title = "Meat protein quantiles",
           ggtheme = theme_minimal(),
           data = flare.cd.cut.df,
           xlab = "Time (days)")$plot
ggsave("plots/meat-cd.pdf",
       print(p),
       width = 9 * 2/3,
       height = 9 * 2/3, units = "in")
...
newdata <- with(flare.cd.cut.df,
                data.frame(Sex = as.factor(rep(Mode(Sex), 3)),
                           Smoke = as.factor(rep(Mode(Smoke, na.rm = TRUE), 3)),
                           PUFA_percEng = quantile(PUFA_percEng, na.rm = TRUE)[2:4]))
...
p <-ggsurvplot(fit,
           ...
           data = flare.cd.cut.df,
           xlab = "Time (days)")$plot
ggsave("plots/pufa-cd.pdf",
       print(p),
       width = 9 * 2/3,
       height = 9 * 2/3, units = "in")

fit.me <- coxph(Surv(time, DiseaseFlareYN) ~
                  Sex + Smoke + PUFA_percEng + frailty(SiteNo),
                control = coxph.control(outer.max = 20),
                data = flare.cd.cut.df)
invisible(cox.summary(fit.me))
```
...
```{R}
if (!dir.exists(paste0(processed.dir, "flares"))) {
  dir.create(paste0(processed.dir, "flares"))
  fileConn <- file(paste0(processed.dir, "flares/README.md"))
  writeLines(c("# README",
             "",
             paste("This directory contains files describing important",
                   "baseline characteristcs for the PREdiCCt cohort alongside", 
                   "time-to-flare data obtained from end-of-study phenotyping", 
                   "(not subject-reported flares via the PREdiCCt mechanism)."),
             "",
             paste0("`all-flares.RDS` uses the total follow-up available,",
                    " whilst `cutoff-flares.RDS` censors any observations after",
                    "two years (the original planned maximum study follow-up)")),
           fileConn)
close(fileConn)
}

saveRDS(flare.df, paste0(processed.dir, "flares/all-flares.RDS"))
saveRDS(flare.cut.df, paste0(processed.dir, "flares/cutoff-flares.RDS"))
```
