...
ggplot(demo, aes(x = IMD, color = as.factor(IMD), fill = as.factor(IMD))) +
  geom_bar() +
  theme_minimal() +
  theme(legend.position = "none") +
...
```{R}
table1(
  ~ Treatment + `Antibiotic use in 6 months` | cat,
  drugs
)
```
...
table1::table1(~ Age + Sex + Ethnicity + IMD + `IBD Duration` + BMI + FC + Biologic | diagnosis2,
  data = demo[, c(-18, -19)],
...
table1(~ Age + Sex + Ethnicity + IMD + diagnosis2 + `IBD Duration` + BMI + FC + Biologic | cat,
  data = demo.fcal,
  render.continuous = my.render.cont
)
```
...
temp <- subset(comp, ParticipantNo %in% FFQ$participantno)
temp$cohort <- "FFQ"
...
comp$cohort <- factor(comp$cohort,
  levels = c("FCAL", "FFQ"),
  labels = c("FCAL cohort", "FFQ cohort")
)
table1(~ Age + Sex + Ethnicity + IMD + diagnosis2 + `IBD Duration` + BMI + FC + Biologic | cohort,
  data = comp,
  render.continuous = my.render.cont,
  overall = FALSE
)
...
```{R}
HBI.df <- read_xlsx(paste0(prefix, "Followup\ form.xlsx"), na = ".") %>%
  select(ParticipantId, MaxHBI)
HBI.df <- subset(HBI.df, ParticipantId %in% demo.cd$ParticipantId)
demo.cd <- merge(demo.cd, HBI.df, by = "ParticipantId", all.x = TRUE)
demo.cd %>%
  ggplot(aes(x = as.factor(MaxHBI))) + geom_bar()
```
...
table1(~ Location + L4 + Behaviour + Perianal + Surgery + Smoke + ECigs | cohort,
  data = comp,
  overall = FALSE
)
```
...
table1(~ Extent + Smoke + ECigs | cohort,
  data = comp,
  overall = FALSE
)
```
