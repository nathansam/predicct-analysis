...
if (file.exists("/.dockerenv")) { # If running in docker
...
temp <- subset(FFQ, participantno %in% fcal$ParticipantNo)
...
```{R}
table1(~ Location + L4 + Behaviour + Perianal + Surgery + Smoke | cat,
       data = demo.cd)
```
...
```{R}
table1(~ Extent + Smoke | cat, data = demo.uc)
```
