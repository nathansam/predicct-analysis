...
available (See [Ethnicity](#ethnicity)), we are unable to adjust the threshold based on
ethnicity.
...
There are more Crohn's disease (CD) than UC/IBDU subjects. This suggests our
inclusion criteria may have biased against UC/IBDU subjects as prevalence for UC
is approximately double that of CD [@Jones_2019].
...
```{R}
#| label: tbl-IMD-fc
#| tbl-cap: "ANOVA between IMD and FC groups."
...
IBD-Control-8 scores were found to be significantly associated with FC.

```{R}
#| label: tbl-control-fc
#| tbl-cap: "ANOVA between IMD and FC groups."
...
```{R}
#| label: fig-VAS-hist
#| fig-cap: "Distribution of IBD-Control-VAS scores. A VAS of 85 or greater is predictive of quiescent disease."
demo %>%
  drop_na(cat) %>%
...
The discretised VAS was found to be significantly associated with FC. 
...
```{R}
pander(summary(aov(IMD ~ cohort, data = comp)))
```
