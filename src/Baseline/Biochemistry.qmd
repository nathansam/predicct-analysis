...
Assumed any Haemoglobin < 50 has been reported in g/dL, any Haemoglobin
...
#| fig-cap: "Distribution of haemoglobin after processing. Vertical red lines indicate the reference range for the respective sex."
#| warning: false
demo <- demo %>%
...
Haemoglobin was found to be significantly associated with FC at a 5%
significance level in male, but not in female, subjects. 

```{R}
#| label: tbl-hb-fc-male
#| tbl-cap: "ANOVA between haemoglobin and FC groups in males."

demo %>%
  filter(Sex == "Male") %>%
  aov(Haemoglobin ~ cat, data = .) %>%
  summary() %>%
  pander()
```

```{R}
#| label: tbl-hb-fc-female
#| tbl-cap: "ANOVA between haemoglobin and FC groups in females."
