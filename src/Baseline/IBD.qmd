...
From @fig-IBD-FC-dist, we can see subjects with UC/IBDU are more likely to have
FC < 50, whiilst CD subjects are more likely to have FC 50-250. Roughtly the
same proportion of subjects with CD and UC/IBDU have FC > 250.

```{R}
#| label: fig-IBD-FC-dist
#| fig-cap: "Distribution of FC category by IBD subtype."
p <- demo %>%
  drop_na(cat) %>%
...
```{R}
#| label: fig-duration-FC-dist-uc
#| fig-cap: "Distribution of disease duration by FC in UC."
p <- demo %>%
  filter(diagnosis2 == "UC/IBDU") %>%
  drop_na(cat) %>%
  mutate(cat = fct_rev(cat)) %>%
...
```{R}
#| label: tbl-control-8
#| tbl-cap: "ANOVA between control-8 and FC groups across the FC cohort."
...
```{R}
#| label: tbl-control-vas
#| tbl-cap: "ANOVA between control VAS and FC groups across the FC cohort."
...
As we are unable to differentiate between participants which are are no
treatment and those for whom treatment data is missing, we will instead
use a `None reported` category.
...
```{R}
#| label: fig-smoke
#| fig-cap: "Smoking status for subjects in the FC cohort."
demo %>%
  drop_na(cat) %>%
...
Smoking status is not associated with FC at recruitment across the entire FC
cohort.

```{R}
#| label: tbl-smoke
#| tbl-cap: "Chi-squared test between between smoking status and FC groups."
...
```{R}
#| label: fig-esmoke
#| fig-cap: "E-cigarette usage for subjects in the FC cohort."
demo.cd %>%
  drop_na(cat) %>%
...
```{R}
#| label: tbl-esmoke
#| tbl-cap: "Chi-squared test between between E-cigarettte usage and FC groups."
...
```{R}
#| label: tbl-pro2-cd
#| tbl-cap: "ANOVA between between PRO2 and FC groups in Crohn's disease."
...
```{R}
#| label: tbl-pro2-uc
#| tbl-cap: "ANOVA between between PRO2 and FC groups in ulcerative colitis/IBDU."
