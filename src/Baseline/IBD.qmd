...
demo %>%
  drop_na(cat) %>%
  ggplot(aes(x = diagnosis2)) +
  geom_bar(fill = "#2B2D42", color = "#1E203A") +
  ylab("Frequency") +
  xlab("IBD type") +
  theme_minimal()
```
...
p <- duration %>%
  ...
  geom_histogram(bins = 25, fill = "#CDEDF6", color = "#5EB1BF") +
  theme_minimal() +
  labs(
    x = "Duration (years)",
    y = "Frequency"
...
p1 <- demo %>%
  drop_na(cat) %>%
  ggplot(aes(x = control_8)) +
  geom_bar(fill = "#9381FF", color = "#745FE3") +
  theme_minimal() +
...
demo %>%
  drop_na(cat) %>%
  ggplot(aes(x = vas_control)) +
  geom_bar(fill = "#D00000", color = "#D00000") +
  theme_minimal() +
  theme(legend.position = "none") +
...
drugs %>%
  ...
  geom_bar(fill = "#52528C", color = "#3A3A61") +
  theme_minimal() +
  theme(legend.position = "none") +
...
demo %>%
  drop_na(cat) %>%
  ggplot(aes(x = Biologic)) +
  geom_bar(fill = "#CACF85", color = "#4A6E3F") +
...
demo.cd %>%
  ...
  geom_bar(fill = "#F9B9B7", color = "#D06965") +
...
areas `r mapping$definition`. L1-L3 were assigned as per convention 
[@silverbergIntegratedClinicalMolecular2005]. If the subject was reported as
only having L4 (no L1-L3), then they were coded as `NA` for the purpose of these
analyses. If no location was reported then location was assigned `NA`. 
...
demo.cd %>%
  ...
  geom_bar(fill = "#C33C54", color = "#80333E") +
...
pander(chisq.test(demo.cd$Location, demo.cd$cat))
```

We also investigated presence of the L4 (upper GI inflammation) modifier. 
...
demo.cd %>%
  drop_na(cat) %>%
  ggplot(aes(x = L4)) +
  geom_bar(fill = "#51E5FF", color = "#079DB1") +
  xlab("Presence of L4") +
  ylab("Frequency") +
  theme_minimal()
```
...
demo.cd %>%
  ...
  geom_bar(fill = "#440381", color = "#340165") +
...
demo.cd %>%
  ...
  geom_bar(fill = "#AF5B5B", color = "#754242") +
...
demo.cd %>%
  drop_na(cat) %>%
  ggplot(aes(x = Smoke)) +
  geom_bar(fill = "#1C448E", color = "#033070") +
  xlab("Smoking status") +
  ylab("Frequency") +
  theme_minimal() 
```
...
demo.cd %>%
  drop_na(cat) %>%
  ggplot(aes(x = ECigs)) +
  geom_bar(fill = "#517664", color = "#385245") +
...
Partial Mayo score was collected via REDCap and is more complete than HBI. 

```{R}
#| label: fig-mayo
#| fig-caption: "Partial Mayo score for UC/IBDU subjects in the FC cohort."
...
demo.uc %>%
  ...
  theme_minimal() +
  ylab("Frequency") +
  xlab("Partial Mayo score at time of recruitment")
```

Partial Mayo score is highly significantly associated with FC. 

```{R}
#| label: tbl-mayo
#| tbl-cap: "ANOVA between between Partial Mayo score and FC groups."
...
(@fig-esmoke-uc) should be considered when interpreting this finding. 
