...
if (file.exists("/docker")) { # If running in docker
  ...
  metadata <- "/Volumes/igmm/cvallejo-predicct/predicct/metadata/"
}
...
site.data <- read.csv(paste0(metadata, "sites.csv"))
sites <- demo %>%
...
ggplot(demo, aes(x = as.factor(month))) +
  geom_bar(fill = "#384E77", color = "#18314F", ) +
  theme_minimal() +
  theme(legend.position = "none") +
