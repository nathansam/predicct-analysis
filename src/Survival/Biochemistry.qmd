...
Due to strong correlation between FC and routinely collected serological
inflammatory markers [@Ishida_2021], FC has not been controlled for in the Cox
models presented on this page. 
...
fit.me <- coxph(
  Surv(softflare_time, softflare) ~
    Sex + IMD + CReactiveProtein + frailty(SiteNo),
...
fit.me <- coxph(
  Surv(softflare_time, softflare) ~
    Sex + IMD + Haemoglobin + frailty(SiteNo),
  control = coxph.control(outer.max = 20),
  data = flare.cd.df,
  x = TRUE
)
...
fit.me <- coxph(
  Surv(softflare_time, softflare) ~
    Sex + IMD + Platelets + frailty(SiteNo),
  control = coxph.control(outer.max = 20),
  data = flare.cd.df,
  x = TRUE
)
...
fit.me <- coxph(
  Surv(softflare_time, softflare) ~
    Sex + IMD + Albumin + frailty(SiteNo),
  control = coxph.control(outer.max = 20),
  data = flare.cd.df,
  x = TRUE
)
