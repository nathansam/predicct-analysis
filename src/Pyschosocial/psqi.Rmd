...
Questions answered by a partner are not calculate in the score.


Component 1: Subjective sleep quality—question 9 (39.6 in PREdiCCt) - "SleepQuality"
Score 0-3
...
Cmponent 2 score = addition of the above /2 (rounded up)
...
Q1 time gone to bed "BedTimeHrs" + "BedTimeMns" - will need to ensure coded into 24 hour clock
...
Component 7: Daytime dysfuntion (questions 7 and 8 = 39.8 and 39.9, "StayingAwake" and "EnoughEnthusiasm")
Both scores 0-3, sum/2 and rounded up

Global PSQI score: sum of seven component scores

A score of 5 or above is indicative of sleep disturbance.
...
#score 5 or above is "poor sleep"
psqi2 <- psqi2 %>%
...
age_counts <- table(psqi2$AgeGroup)
age_counts_df <- as.data.frame(age_counts)
colnames(age_counts_df) <- c("Age_Category", "Count")
age_counts_df
```
...
```{r, join smoking, previous flare, fc}
...
psqi_IBD$flare_group <- factor(psqi_IBD$flare_group, levels = c( "No Flares","1 or More Flares"))
...
save_path <- "/Volumes/igmm/cvallejo-predicct/people/chiara/"
saveRDS(psqi_IBD, paste0(save_path, "psqi_IBD.RDS"))
```


Sleep disorder distribution (Sleep disturbance =  PSQI score => 5)
All analyses were carried out with chi-square test unless otherwise specified

All patients: 
* Sex: females have a statistically increased risk of sleep disturbance (p-value > 0.00001 )
* IBD type: There is no significant association between IBD type and sleep disturbance (p-value = 0.05), although rates appear higher in Crohn's (see table)
* Flares in last year: is a significant association between flares in last year and presence of sleep disturbance (p-value = 0.02)
...
# Flare last year
contingency_table <- table(psqi_IBD$flare_group, psqi_IBD$SleepDisturbance)
#chi.sq__
chi_squared_result <- chisq.test(contingency_table)
print(chi_squared_result)
# Check if the result is significant at a certain alpha level (e.g., 0.05)
...
psqi2_percent <- psqi_IBD %>%
  group_by(TotalScore, flare_group) %>%
  summarise(Percentage = n() / nrow(psqi_IBD) * 100) %>%
  ungroup()
...
contingency_table <- table(psqi_IBD$cat, psqi_IBD$SleepDisturbance)
contingency_table
...
contingency_table <- table(psqi_IBD$AgeGroup, psqi_IBD$SleepDisturbance)
contingency_table
options(expressions = 10000)
fisher_result <- fisher.test(contingency_table, workspace = 2e7, hybrid = TRUE, control = list(worksize = 1e7))
fisher_result
...
Divide into UC/IBDU and CD and add disease location data
``` {r, divide into disease type datasets}
psqi_UC <- psqi_IBD[psqi_IBD$diagnosis2 == "UC/IBDU",]
psqi_CD <- psqi_IBD[psqi_IBD$diagnosis2 == "CD",]
```

Sleep disorder distribution (Sleep disturbance =  PSQI score => 5)
All analyses were carried out with chi-square test unless otherwise specified, * = Fisher's Exact test

CD patients:
* Sex: females have a statistically increased risk of sleep disturbance (p-value = 0.0026 )
* Flares in last year: there is a significant association between flares in last year and presence of sleep disturbance (p-value = 0.001)
* FC category* (<50, 20-250, >250): no statistically significant difference (p-value = 0.55) 
* Age*: Patients at extremes of age have higher rates of sleep disturbance (p-value = 0.0005) (highert in 18-24. followed by 55-64)
...
Sleep disorder distribution (Sleep disturbance =  PSQI score => 5)
All analyses were carreied out with chi-square test unless otherwise specified, * = Fisher's Exact test

UC/IBDU patients:
* Sex: females have a statistically increased risk of sleep disturbance (p-value = 0.0005), stronger effect than in CD
* Flares in last year: there is NO significant association between flares in last year and presence of sleep disturbance (p-value = p-value = 0.67) - Different to CD which IS significant
* FC category* (<50, 20-250, >250): no statistically significant difference (p-value = 0.424), as in CD
* Age*: Patients at extremes of age have higher rates of sleep disturbance (p-value = 0.001), less strong than CD. Highest rates in 45-54 age group, followed by 18-24.


``` {r, psqi_ UC statistics baseline}
# Sex
...
* Significant controlling for site, sex, smoking, IMD, Age group
* Significant when controlling for FC additionally (p-value 0.003, 95% CI 1.09-1.56)
...
fit.me <- coxph(Surv(time, DiseaseFlareYN) ~
                  Sex + Smoke + AgeGroup + IMD + SleepDisturbance + frailty(SiteNo),
                data = psqi_CD_soft)
cox.summary(fit.me)
summary(cox_model)
