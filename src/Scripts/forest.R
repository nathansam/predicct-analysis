...
p1 <- clin.forest %>%
  ...
  guides(
    color = guide_legend(title = "Colour Legend",
                         override.aes = list(shape = 15, linetype = 0)),
    shape = guide_legend(title = "IBD Diagnosis",
...
clin.forest$CI <- paste0(round(clin.forest$Lower95, 2),
                         "--",
...
for (var in unique(clin.forest$Variable)) {
  temp.df <- data.frame(Variable = var,
                        HR = NA,
                        Lower95 = NA,
                        Upper95 = NA,
                        p = NA,
  ...
  temp <- subset(clin.forest,
                 Variable == var & Diagnosis == "CD")[, c(-5, -8)]
  if (nrow(temp) > 0) {
    temp.df <- rbind(temp.df, cbind(
      data.frame(Variable = "  CD"), temp))
  }


  temp <- subset(clin.forest,
                 Variable == var & Diagnosis == "UC/IBDU")[, c(-5, -8)]

  if (nrow(temp) > 0) {
    temp.df <- rbind(temp.df, cbind(
      data.frame(Variable = "  UC/IBDU"), temp))
  }
...
cairo_pdf("plots/arranged/clin-forest.pdf",
          width = 8.3,
          height = 13)
forestplot(
...
png("plots/arranged/clin-forest.png",
    width = 8.3,
    height = 13,
    units = "in",
    res = 300)
