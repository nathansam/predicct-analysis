...
Participants \<18 years old (n = `r n_underage`) and participants missing baseline faecal calprotectin (FC; n = `r n_missing_fc`) were excluded. Participants missing HADS data (n = `r n_missing_hads`) were also excluded.
...
This data is saved as "hads_with_demographics" (separate columns for anxiety and depression scores) and "hads_for_analysis" with variable `hads_type` used to identify whether the score is for depression or anxiety (multiple rows per participant).

We tested for associations of diagnosis type (CD vs UC/IBDU), sex, age group, flares reported in the previous year (flare vs no flares) and faecal calprotectin (by categories: \<50, 50-250, \>250) with score groups for anxiety and depression separately. This was done for the PREdiCCt IBD cohort as a whole, and for CD and UC/IBDU groups separately. Testing for associations was done by chi-square test or Fisher's exact test, where appropriate.

### Baseline scores

At baseline, anxiety scores tended to be higher than depression scores.

A recent meta-analysis by @Barberio2021 looking at the prevalence of anxiety and depression in people with IBD found that

-   Overall, pooled prevalence of anxiety symptoms was 32·1% (95% CI 28·3-36·0) in 58 studies ($I^2$=96·9%) and pooled prevalence of depression symptoms was 25·2% (22·0-28·5) in 75 studies ($I^2$=97·6%).
-   In studies that reported prevalence of anxiety or depression in patients with Crohn's disease and ulcerative colitis within the same study population, patients with Crohn's disease had higher odds of anxiety symptoms (OR 1·2, 95% CI 1·1-1·4) and depression symptoms (1·2, 1·1-1·4) than patients with ulcerative colitis.
-   Overall, women with IBD were more likely to have symptoms of anxiety than were men with IBD (pooled prevalence 33·8% \[95% CI 26·5-41·5\] for women vs 22·8% \[18·7-27·2\] for men; OR 1·7 \[95% CI 1·2-2·3\]). They were also more likely to have symptoms of depression than men were (pooled prevalence 21·2% \[95% CI 15·4-27·6\] for women vs 16·2% \[12·6-20·3\] for men; OR 1·3 \[95% CI 1·0-1·8\]).

Our findings also show higher rates of anxiety compare to depression, and higher rates of anxiety in women compared to men. However we did not find any difference between sexes in rates of depression. We also did not find any difference across diagnosis (Crohn's vs UC/IBDU).
...
filtered_data_anxiety <- hads_for_analysis %>% 
...
#depression_hads
filtered_data_depression <- hads_for_analysis %>% 
...
#UC anxiety
anxiety_uc <- filtered_data_anxiety %>%
  dplyr::filter(diagnosis2 == "UC/IBDU")
#UC depression
depression_uc <- filtered_data_depression %>%
  dplyr::filter(diagnosis2 == "UC/IBDU")
#CD anxiety
anxiety_cd <- filtered_data_anxiety %>% 
  dplyr::filter(diagnosis2 == "CD")
#CD depression
depression_cd <- filtered_data_depression %>% 
  dplyr::filter(diagnosis2 == "CD")
```
...
Note: add in the phrase "at the 95% confidence level". Comment on the magnitude of the differences between groups?
...
-   significant association with flares in previous year whole cohort and CD, but not true for UC/IBDU group
-   no association with FC
...
filtered_data_anxiety %>%
  dplyr::group_by(score_group, diagnosis2) %>%
  dplyr::count() %>%
  dplyr::group_by(diagnosis2) %>%
  dplyr::mutate(p = n/sum(n)) %>%
  ggplot() +
  geom_col(aes(x = score_group, y = p, fill = diagnosis2), position = 'dodge')
...
if (chi_squared_result_anxiety$p.value < alpha) {
  cat("There is a significant association between diagnosis type and anxiety score groups at the", (1-alpha)*100, "% confidence level.\n")
} else {
  cat("There is no significant association between diagnosis type and anxiety score groups at the", (1-alpha)*100, "% confidence level.\n")
}
...
if (chi_squared_result_depression$p.value < alpha) {
  cat("There is a significant association between diagnosis type and score group in depression HADS at the", (1-alpha)*100, "% confidence level.\n")
} else {
  cat("There is no significant association between diagnosis type and depression score groups at the", (1-alpha)*100, "% confidence level.\n")
}
...
alpha <- 0.05
if (chi_squared_result_anxiety$p.value < alpha) {
  cat("There is a significant association between sex and anxiety score groups at the", (1-alpha)*100, "% confidence level.\n")
} else {
  cat("There is no significant association between sex and anxiety score groups at the", (1-alpha)*100, "% confidence level.\n")
}
chi_squared_result_anxiety %>%
  pander()
```
...
if (chi_squared_result_depression$p.value < alpha) {
  cat("There is a significant association between sex and score group in depression HADS at the", (1-alpha)*100, "% confidence level.\n")
} else {
  cat("There is no significant association between sex and depression score groups at the", (1-alpha)*100, "% confidence level.\n")
}
chi_squared_result_depression %>%
  pander()
```
...
if (chi_squared_result_anxiety$p.value < alpha) {
  cat("There is a significant association between age groups and anxiety score groups at the", (1-alpha)*100, "% confidence level.\n")
} else {
  cat("There is no significant association between age groups and anxiety score groups at the", (1-alpha)*100, "% confidence level.\n")
}
...
if (chi_squared_result_dep$p.value < alpha) {
  cat("There is a significant association between age groups and depression score groups at the", (1-alpha)*100, "% confidence level.\n")
} else {
  cat("There is no significant association between age groups and depression score groups at the", (1-alpha)*100, "% confidence level.\n")
}
...
ggplot(filtered_data_anxiety, aes(x = age, y = hads_score)) +
  geom_jitter(alpha = 0.2) +  # Scatter plot, add alpha (transparency)
...
correlation_result_age_anxiety <- lm(hads_score ~ age,
                                     data = filtered_data_anxiety)
...
```{R}
correlation_result_age_depression <- lm(hads_score ~ age,
                                        data = filtered_data_depression)
...
::: callout-note
We need to do plots to prove directionality of this relationship (done for whole cohort, not for UC/IBDU and CD individually)
:::
...
#| results: "hold"
# TO-DO add table caption
contingency_cd_anx <- table(anxiety_cd$flare_group, 
                            anxiety_cd$score_group)
...
#| results: "hold"
# TO-DO add table caption
contingency_uc_anx <- table(anxiety_uc$flare_group, 
                            anxiety_uc$score_group)
...
#| results: "hold"
# TO-DO add table caption
contingency_uc_dep <- table(depression_uc$flare_group, 
                            depression_uc$score_group)
...
#| fig-cap: "Distribution of baseline HADS-Anxiety scores per flare group"
...
hads_control %<>% dplyr::mutate(control_grouped = factor(control_grouped))
hads_control %<>% dplyr::mutate(vas_control = factor(vas_control))

filtered_data_anxiety <- hads_control %>% 
  dplyr::filter(hads_type == "anxiety_hads")

filtered_data_depression <- hads_control %>% 
  dplyr::filter(hads_type == "depression_hads")
```
...
data_flare_b <- data_flare_a %>% 
  dplyr::filter(diagnosis2 == "UC/IBDU")

contingency_table_b <- table(data_flare_b$score_group, 
                             data_flare_b$control_grouped)
...
# TO-DO add table caption
#depression, all
data_flare_d <- hads_control %>% 
  dplyr::filter(hads_type == "depression_hads")

contingency_table_flare <- table(data_flare_d$score_group, 
                                 data_flare_d$control_grouped)
...
# TO-DO add table caption
#depression, uc/ibdu
data_flare_e <- data_flare_d %>% 
  dplyr::filter(diagnosis2 == "UC/IBDU")

contingency_table_e <- table(data_flare_e$score_group, 
                             data_flare_e$control_grouped)
