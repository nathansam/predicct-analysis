...
# Excluding subjects who say they do moderate work but for 0 days (they do have an hour though)
exercise <- exercise %>%
...
**Exercise Achievement Rates**: - Approximately **`r round(100 * sum(exercise$MinimumExercise == "Yes") / nrow(exercise), 1)`%** of participants meet WHO minimum exercise recommendations.
...
#| echo: false
#| message: false
ex_UC <- exercise %>% dplyr::filter(diagnosis2 == "UC/IBDU")
ex_CD <- exercise %>% dplyr::filter(diagnosis2 == "CD")
...
if (chi_squared_result$p.value < alpha) {
  message(
    ...
    " minimum exercise at the 95% confidence level."
  )
} else {
  message(
...
-   No association between Sex and alcohol consumption in CD subset, p-value = 0.79
-   No association between flares in previous year and alcohol consumption, p-value = 0.12
-   Significant association between age group and alcohol consumption intake, p-value = 0.002
-   No association between smoking and alcohol consumption intake, p-value = 0.549
-   No association between FC and alcohol consumption intake, p-value = 0.56
...
-   There is a significant association between sex and alcohol consumption in UC patients, p-value \>0.00001
