...
exercise <- exercise %>% 
  mutate(LackExercise = ifelse(VigorousWeek < 75 & ModerateWeek < 150 & METs < 600, "Yes", "No"))
...
#site, sex, IMD
cox_model <- coxph(Surv(time, Cens) ~ MinimumExercise + Sex + IMD + frailty(SiteNo), data = exercise_soft)
summary(cox_model)

#control for fc in exercise
cox_model <- coxph(Surv(time, Cens) ~ MinimumExercise + Sex + IMD + cat + frailty(SiteNo), data = exercise_soft)
summary(cox_model)
...
#sex and frailty
cox_model <- coxph(Surv(time, Cens) ~ MinimumExercise + Sex + IMD + frailty(SiteNo), data = exercise_hard)
summary(cox_model)

#control for fc additionally
cox_model <- coxph(Surv(time, Cens) ~ MinimumExercise + Sex + IMD + cat + frailty(SiteNo), data = exercise_hard)
summary(cox_model)
```
...
cox_model <- coxph(Surv(time, Cens) ~ LackExercise + frailty(SiteNo), data = ex_CD_soft)
cox.summary(cox_model)
...
table1::table1(~ Sex + AgeGroup + flare_group + cat | weekly_units,
  data = ETOH_UC,
  render.continuous = my.render.cont,
  caption = "Distribution of patients across alcohol intake groups in UC"
)
...
Patient-reported flare - no significant associations
...
cox_model <- coxph(Surv(time, Cens) ~ weekly_units + Sex + IMD + AgeGroup + frailty(SiteNo), data = ETOH_soft)
summary(cox_model)

#control for FC additioally
cox_model <- coxph(Surv(time, Cens) ~ weekly_units + Sex + IMD + cat + AgeGroup + frailty(SiteNo), data = ETOH_soft)
summary(cox_model)
...
#controlling for Sex, frailty, age, IMD
cox_model <- coxph(Surv(time, Cens) ~ weekly_units + Sex + AgeGroup + IMD + frailty(SiteNo), data = ETOH_CD_hard)
summary(cox_model)

#control for FC additionally
cox_model <- coxph(Surv(time, Cens) ~ weekly_units + Sex + cat + IMD + AgeGroup + frailty(SiteNo), data = ETOH_CD_hard)
summary(cox_model)
```
...
cox_model <- coxph(Surv(time, Cens) ~ AlcoholIntake + frailty(SiteNo), data = ETOH_CD_soft)
cox.summary(cox_model)

#controlling for Sex, frailty, age, IMD
cox_model <- coxph(Surv(time, Cens) ~ weekly_units + Sex + IMD + AgeGroup + frailty(SiteNo), data = ETOH_CD_soft)
summary(cox_model)

#control for FC additioally
cox_model <- coxph(Surv(time, Cens) ~ weekly_units + Sex + IMD + cat + AgeGroup + frailty(SiteNo), data = ETOH_CD_soft)
cox.summary(cox_model)

#new levels - controlling for FC
cox_model <- coxph(Surv(time, Cens) ~ AlcoholIntake + Sex + IMD + cat + AgeGroup + frailty(SiteNo), data = ETOH_CD_soft)
cox.summary(cox_model)
...
cox_model <- coxph(Surv(time, Cens) ~ AlcoholIntake + frailty(SiteNo), data = ETOH_CD_hard)
cox.summary(cox_model)
...
cox_model <- coxph(Surv(time, Cens) ~ AlcoholIntake + Sex + cat + IMD + AgeGroup + frailty(SiteNo), data = ETOH_CD_hard)
cox.summary(cox_model)
```
...
#univariate - new levels
cox_model <- coxph(Surv(time, Cens) ~ AlcoholIntake + frailty(SiteNo), data = ETOH_UC_soft)
cox.summary(cox_model)

#controlling for Sex, frailty, age, IMD
cox_model <- coxph(Surv(time, Cens) ~ weekly_units + Sex + IMD + AgeGroup + frailty(SiteNo), data = ETOH_UC_soft)
summary(cox_model)

#control for FC additioally
cox_model <- coxph(Surv(time, Cens) ~ weekly_units + Sex + cat + IMD + AgeGroup + frailty(SiteNo), data = ETOH_UC_soft)
cox.summary(cox_model)

#control for FC additioally - new levels
cox_model <- coxph(Surv(time, Cens) ~ AlcoholIntake + Sex + cat + IMD + AgeGroup + frailty(SiteNo), data = ETOH_UC_soft)
cox.summary(cox_model)
```
...
#univariate - new levels
cox_model <- coxph(Surv(time, Cens) ~ AlcoholIntake + frailty(SiteNo), data = ETOH_UC_hard)
cox.summary(cox_model)

#controlling for Sex, frailty, age, IMD
cox_model <- coxph(Surv(time, Cens) ~ weekly_units + Sex + IMD + AgeGroup + frailty(SiteNo), data = ETOH_UC_hard)
summary(cox_model)

#control for FC additinoally
cox_model <- coxph(Surv(time, Cens) ~ weekly_units + Sex + cat + IMD + AgeGroup + frailty(SiteNo), data = ETOH_UC_hard)
cox.summary(cox_model)
