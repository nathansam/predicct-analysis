...
knitr::purl("/Users/arudge/GitHub/predicct-analysis/src/Psychosocial/Alex Rudge/Primary analysis/Life Events/Life Events interactions.qmd", output = qmd_code, documentation = 0)
source(qmd_code, local = .GlobalEnv)
unlink(qmd_code)

source("~/GitHub/predicct-analysis/src/Psychosocial/Alex Rudge/functions.R")

# Extract AnyLifeEvents x diagnosis2 interaction terms from the life-events Cox models.
...
readr::write_rds(
  x = cox_results_lifeevents_interaction_age_mice,
  file = paste0(filepath, "cox_results_lifeevents_interaction_age_mice.rds")
)
