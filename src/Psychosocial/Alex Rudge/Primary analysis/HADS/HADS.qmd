...
Patients completed the Hospital Anxiety and Depression Scale upon their recruitment to the study (baseline). In this analysis, our aim was to determine the extent to which anxiety, and depression scores are associated with subsequent patient-reported and objective flares.

Note that in the code, patient-reported and objective flares may be referred to as soft and hard flares, respectively.
...
- Combine data from different sources: HADS questionnaire responses, demographics, faecal calprotectin (FC) measurements, index of multiple deprivation (IMD), flare history, and IBD control scores.

- Remove participants under 18 years of age.

- Calculate overall anxiety and depression scores based on patient questionnaire responses. Note that in the raw HADS data, the questionnaire responses are coded from 1 to 4, and some are coded inversely (1 is the 'bad' response and 4 the 'good' response). Therefore, to calculate the HADS score the responses were rescaled to between 0 and 3, and inversed where appropriate. The overall anxiety and depression scores were then the sum of the responses.

- We categorise the anxiety and depression scores into 0-7 (normal) and 8-21 (elevated) for analysis.

- We exclude patients for whom we could not calculate the HADS score.
...
# Join the common participant-level data.
hads %<>%
  dplyr::left_join(data_common, by = "ParticipantNo")
...
We visualise the amount of missingness in the data and find that the primary source of missingness was due to missing FC measurements.
...
Associations between anxiety and depression scores and the rates of patient-reported and objective flares are assessed using multivariable Cox proportional hazard models. Each model adjusted for biological sex, age, smoking status, IMD, and FC (natural log transformed), and included hospital site as a frailty term. Continuous variables (age and FC) were modeled using natural cubic splines, with degrees of freedom determined using likelihood ratio tests at a 95% significance level. Participants with missing flare status or time are excluded.
...
# Spline for anxiety
summon_lrt(cox_anxiety_soft, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
# Spline for depression
summon_lrt(cox_depression_soft, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
##### Anxiety {.unlisted .unnumbered}

```{r}
# Depression
plot_continuous_hr(
  data = data_survival_anxiety_soft, 
  model = cox_anxiety_soft, 
  variable = 'hads_score')

```

##### Depression {.unlisted .unnumbered}

```{r}
# Anxiety
plot_continuous_hr(
  data = data_survival_depression_soft, 
  model = cox_depression_soft, 
  variable = 'hads_score')
...
# Spline for anxiety
summon_lrt(cox_anxiety_hard, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
# Spline for depression
summon_lrt(cox_depression_hard, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
# Spline for anxiety
summon_lrt(cox_anxiety_soft_uc, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
# Spline for depression
summon_lrt(cox_depression_soft_uc, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
# Spline for anxiety
summon_lrt(cox_anxiety_hard_uc, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
# Spline for depression
summon_lrt(cox_depression_hard_uc, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
# Spline for anxiety
summon_lrt(cox_anxiety_soft_cd, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
# Spline for depression
summon_lrt(cox_depression_soft_cd, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
# Spline for anxiety
summon_lrt(cox_anxiety_hard_cd, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
# Spline for depression
summon_lrt(cox_depression_hard_cd, remove = 'hads_score', add = 'ns(hads_score, df = 2)')
# No
...
# Cox model with MICE
# Run the mice
cox_anxiety_cts_soft_uc_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    hads_score +
    IMD +
    Sex +
...
# Fit the Cox
cox_anxiety_cts_soft_uc_pool <- with(cox_anxiety_cts_soft_uc_mice,
     coxph(
       Surv(time, DiseaseFlareYN) ~
         hads_score +
         IMD +
...
```{r}
cox_anxiety_cts_hard_uc <- coxph(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + ns(age_decade, df = 4) + ns(FC, df = 2) +
    Smoke + frailty(SiteNo),
  data = data_survival_anxiety_hard_uc
)

cox_anxiety_cts_hard_uc %>% print_survival()
```

###### MICE

```{r}
# Note: mice does not support splines during imputation.
cox_anxiety_cts_hard_uc_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_anxiety_hard_uc,
  m = 20,
  printFlag = FALSE
)

cox_anxiety_cts_hard_uc_pool <- with(
  cox_anxiety_cts_hard_uc_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      hads_score + IMD + Sex + ns(age_decade, df = 4) + ns(FC, df = 2) +
      Smoke + frailty(SiteNo)
...
```{r}
cox_anxiety_cts_soft_cd <- coxph(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_anxiety_soft_cd
)

cox_anxiety_cts_soft_cd %>% print_survival()
```

###### MICE

```{r}
cox_anxiety_cts_soft_cd_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_anxiety_soft_cd,
  m = 20,
...
cox_anxiety_cts_soft_cd_pool <- with(
  cox_anxiety_cts_soft_cd_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo)
  )
...
```{r}
cox_anxiety_cts_hard_cd <- coxph(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_anxiety_hard_cd
)

cox_anxiety_cts_hard_cd %>% print_survival()
```

###### MICE

```{r}
cox_anxiety_cts_hard_cd_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_anxiety_hard_cd,
  m = 20,
...
cox_anxiety_cts_hard_cd_pool <- with(
  cox_anxiety_cts_hard_cd_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo)
  )
...
```{r}
cox_depression_cts_soft_uc <- coxph(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_depression_soft_uc
)

cox_depression_cts_soft_uc %>% print_survival()
```

###### MICE

```{r}
cox_depression_cts_soft_uc_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_depression_soft_uc,
  m = 20,
...
cox_depression_cts_soft_uc_pool <- with(
  cox_depression_cts_soft_uc_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo)
  )
...
cox_depression_cts_soft_uc_pool %>%
  broom::tidy(conf.int = TRUE, conf.level = 0.95, exponentiate = TRUE) %>%
  print_survival_mice()
```
:::
...
```{r}
cox_depression_cts_hard_uc <- coxph(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + ns(age_decade, df = 4) + ns(FC, df = 2) +
    Smoke + frailty(SiteNo),
  data = data_survival_depression_hard_uc
...
cox_depression_cts_hard_uc %>% print_survival()
```
...
```{r}
# Note: mice does not support splines during imputation.
cox_depression_cts_hard_uc_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_depression_hard_uc,
  m = 20,
...
cox_depression_cts_hard_uc_pool <- with(
  cox_depression_cts_hard_uc_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      hads_score + IMD + Sex + ns(age_decade, df = 4) + ns(FC, df = 2) +
      Smoke + frailty(SiteNo)
...
cox_depression_cts_hard_uc_pool %>%
  broom::tidy(conf.int = TRUE, conf.level = 0.95, exponentiate = TRUE) %>%
  print_survival_mice()
```
:::
...
```{r}
cox_depression_cts_soft_cd <- coxph(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_depression_soft_cd
)

cox_depression_cts_soft_cd %>% print_survival()
```

###### MICE

```{r}
cox_depression_cts_soft_cd_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_depression_soft_cd,
  m = 20,
...
cox_depression_cts_soft_cd_pool <- with(
  cox_depression_cts_soft_cd_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo)
  )
...
cox_depression_cts_soft_cd_pool %>%
  broom::tidy(conf.int = TRUE, conf.level = 0.95, exponentiate = TRUE) %>%
  print_survival_mice()
```
:::
...
```{r}
cox_depression_cts_hard_cd <- coxph(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_depression_hard_cd
)

cox_depression_cts_hard_cd %>% print_survival()
```

###### MICE

```{r}
cox_depression_cts_hard_cd_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo),
  data = data_survival_depression_hard_cd,
  m = 20,
...
cox_depression_cts_hard_cd_pool <- with(
  cox_depression_cts_hard_cd_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      hads_score + IMD + Sex + age_decade + FC + Smoke + frailty(SiteNo)
  )
...
cox_depression_cts_hard_cd_pool %>%
  broom::tidy(conf.int = TRUE, conf.level = 0.95, exponentiate = TRUE) %>%
  print_survival_mice()
```
:::
