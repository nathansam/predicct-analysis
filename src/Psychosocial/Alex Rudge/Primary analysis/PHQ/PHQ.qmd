...
Participants completed the Patient Health Questionnaire-15 (PHQ) upon their recruitment to the study, which was used to assess the patient's physical somatisation severity. In this analysis, our aim was to determine the extent to which somatisation is associated with subsequent patient-reported and objective flares.
...
- Remove participants who did not answer all PHQ questions.

- Rescale the PHQ responses, as the raw data responses were recorded as 1-4 instead of 0-3.

- Calculate the somatisation score by summing responses from each PHQ question.

- Categorise somatisation score into 0-4 (none), 5-9 (mild), and 10-30 (moderate to severe).
...
::: panel-tabset
```{r}
# Select PHQ data and UC patients
...
::: panel-tabset
```{r}
# Select PHQ data and CD patients
...
# Spline for Total PHQ score
summon_lrt(
  cox_totalphq_soft,
  remove = 'TotalPHQ',
  add = 'ns(TotalPHQ, df = 2)'
)
...
##### Total PHQ score {.unlisted .unnumbered}

```{r}
# Total PHQ score
plot_continuous_hr(
  data = data_survival_soft,
  model = cox_totalphq_soft,
  variable = 'TotalPHQ')
```
:::
