...
# Called hads_for_analysis, rename to data_hads
data_hads <- hads_for_analysis
...
# Anxiety
data_baseline_anxiety <- data_hads %>%
  dplyr::filter(hads_type == 'anxiety_hads')

# Depression
data_baseline_depression <- data_hads %>%
...
ggplot(data_hads, aes(x = hads_score, colour = hads_type)) +
...
summon_chisq_test(data = data_baseline_anxiety, dependent = dependent, independent = independent)
