...
control %<>%
  ...
  dplyr::mutate(
    control_8_grouped = dplyr::case_when(
      control_8 >= 13 ~ ">=13",
      control_8 < 13 ~ "<13",
      TRUE ~ NA_character_
    ),
    control_8_grouped = factor(
      control_8_grouped,
      levels = c("<13", ">=13")
    )
  )
...
plot_soft_pre <- data_soft_long_counts %>%
  dplyr::filter(
    ...
    at_risk_flag == TRUE) %>%
  dplyr::mutate(
    control_8_grouped = forcats::fct_na_value_to_level(control_8_grouped, "Missing"),
    control_8_grouped = forcats::fct_relevel(control_8_grouped, "Missing", "<13", ">=13")
  ) %>%
  dplyr::count(month, control_8_grouped) %>%
  # Calculate percentage
  dplyr::group_by(month) %>%
  dplyr::mutate(
    p = n/sum(n[control_8_grouped != "Missing"]),
  ...
  dplyr::mutate(
    percent = dplyr::case_when(
      control_8_grouped == "Missing" ~ NA,
      TRUE ~ percent
    )
  ) %>%
  ggplot(aes(x = month, y = n, fill = control_8_grouped)) +
...
data_soft_td <- data_soft_long %>%
  dplyr::select(
    ParticipantNo, month, control_8
  ) %>%
  # Last observation carried forward
  dplyr::arrange(ParticipantNo, month) %>%
  dplyr::group_by(ParticipantNo) %>%
  tidyr::fill(control_8, .direction = "down") %>%
