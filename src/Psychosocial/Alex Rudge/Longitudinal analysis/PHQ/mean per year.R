...
# 12 to 24 months
data_soft_long_12to24 <- data_soft_long %>%
...
# 12 to 24 months
data_hard_long_12to24 <- data_hard_long %>%
  # Filter anyone flaring or censored before 12 months
  dplyr::filter(time > 365) %>%
  # Flare between 12 and 24
  dplyr::mutate(
...
data_hard_long_0to12 %>%
  dplyr::bind_rows(data_hard_long_12to24) %>%
  dplyr::mutate(group = interaction(flare_status, segment)) %>%
  ggplot(aes(
    x = month, 
    y = mean_val, 
