...
for (suffix in c("_cc.rds", "_mice.rds")) {
  ...
  # Continuous HADS and PHQ results
  cox_results_hads_anxiety <- readr::read_rds(
    paste0(filepath, "cox_results_hads_anxiety_cts", suffix)
  )

  cox_results_hads_depression <- readr::read_rds(
    paste0(filepath, "cox_results_hads_depression_cts", suffix)
  )

  cox_results_phq <- readr::read_rds(
    paste0(filepath, "cox_results_phq_cts", suffix)
  )

  # Binary variables retain their existing result files
  cox_results_exercise <- readr::read_rds(
    paste0(filepath, "cox_results_exercise", suffix)
  )

  cox_results_lifeevents <- readr::read_rds(
    paste0(filepath, "cox_results_lifeevents", suffix)
  )

  cox_results_psqi <- readr::read_rds(
    paste0(filepath, "cox_results_psqi", suffix)
  )

  cox_results_fatigue <- readr::read_rds(
    paste0(filepath, "cox_results_fatigue", suffix)
  )
  ...
  # Benjamini-Hochberg adjustment, separately for UC/IBDU and CD
  cox_results %<>%
