...
phq <- phq %>%
  left_join(data_common, by = "ParticipantNo")
...
# Statistical tests for each independent variable with somatisation
# With p-values adjusted for multiple testing using FDR correction.
...
```{r}
# Select somatisation data and UC patients
data_baseline_uc <- phq %>%
...
```{r}
# Select somatisation data and CD patients
data_baseline_cd <- phq %>%
...
```{r}
# Survival data
# Join phq with flare data
# Patient reported
data_survival_soft <- phq %>% dplyr::inner_join(
...
# Hard flares
data_survival_hard <- phq %>% dplyr::inner_join(
...
The categorical somatisation analysis is repeated using the continuous PHQ-15 score (`TotalPHQ`) as the primary exposure.

## Results - continuous PHQ score
...
```{r}
# Cox model using the continuous PHQ score
cox_phq_cts_soft <- coxph(
  Surv(time, DiseaseFlareYN) ~
    TotalPHQ +
    IMD +
    Sex +
...
```{r}
# Cox model with MICE
cox_phq_cts_soft_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    TotalPHQ +
    IMD +
    Sex +
...
cox_phq_cts_soft_pool <- with(
  cox_phq_cts_soft_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      TotalPHQ +
      IMD +
...
```{r}
# Cox model with MICE
cox_phq_cts_hard_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    TotalPHQ +
    IMD +
    Sex +
...
cox_phq_cts_hard_pool <- with(
  cox_phq_cts_hard_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      TotalPHQ +
      IMD +
...
```{r}
# Cox model with MICE
cox_phq_cts_soft_uc_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    TotalPHQ +
    IMD +
    Sex +
...
cox_phq_cts_soft_uc_pool <- with(
  cox_phq_cts_soft_uc_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      TotalPHQ +
      IMD +
...
```{r}
# MICE does not support splines during imputation, so use the
# unsplined variables for the imputation model.
cox_phq_cts_hard_uc_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    TotalPHQ +
    IMD +
    Sex +
...
cox_phq_cts_hard_uc_pool <- with(
  cox_phq_cts_hard_uc_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      TotalPHQ +
      IMD +
...
```{r}
# Cox model with MICE
cox_phq_cts_soft_cd_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    TotalPHQ +
    IMD +
    Sex +
...
cox_phq_cts_soft_cd_pool <- with(
  cox_phq_cts_soft_cd_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      TotalPHQ +
      IMD +
...
```{r}
# Cox model with MICE
cox_phq_cts_hard_cd_mice <- coxph_mice(
  Surv(time, DiseaseFlareYN) ~
    TotalPHQ +
    IMD +
    Sex +
...
cox_phq_cts_hard_cd_pool <- with(
  cox_phq_cts_hard_cd_mice,
  coxph(
    Surv(time, DiseaseFlareYN) ~
      TotalPHQ +
      IMD +
