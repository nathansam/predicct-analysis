...
participants <- readr::read_rds(
  "/Volumes/igmm/cvallejo-predicct/people/Alex/Predicct2/Data/participants.rds"
)
...
```{r}
# Select fatigue data and UC patients
...
```{r}
# Select fatigue data and CD patients
...
# Kaplan Meier
data_survival_soft %>%
  survfit(Surv(time, DiseaseFlareYN) ~ OftenLackEnergy, data = .) %>%
  ggsurvplot(
    ., 
    data = data_survival_soft, 
    conf.int = TRUE, 
