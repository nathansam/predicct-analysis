...
# Remove patients with missing ParticipantId
...
# Exclude missing life events (these patients simply did not fill out the questionnaire)
lifeevents <- lifeevents %>% 
...
data_vis_missingness <- lifeevents %>%
...
# Statistical tests for each independent variable with AnyLifeEvents
...
```{r}
# Select life events data and UC patients
...
```{r}
# Select life events data and CD patients
...
```{r}
# Survival data
# Join lifeevents with flare data
# Patient reported
data_survival_soft <- lifeevents %>% dplyr::inner_join(
...
# Hard flares
data_survival_hard <- lifeevents %>% dplyr::inner_join(
...
UC, soft, control df = 2. Hard, age df = 3, FC df = 2.
...
data_survival_soft %>%
  ...
  ggsurvplot(
    ...
    pval = TRUE,
    pval.method = TRUE,
    legend.title = 'Minimum lifeevents',
