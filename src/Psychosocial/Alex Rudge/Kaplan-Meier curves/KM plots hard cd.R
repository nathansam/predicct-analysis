...
# 2x2 plot for the manuscript for the main variables in hard cd
...
plot_anxiety <- readr::read_rds(
  file = paste0(filepath, "plot_anxiety_hard_cd.rds")
)
...
plot_depression <- readr::read_rds(
  file = paste0(filepath, "plot_depression_hard_cd.rds")
)
...
plot_psqi <- readr::read_rds(
  file = paste0(filepath, "plot_psqi_hard_cd.rds")
)
...
plot_phq <- readr::read_rds(
  file = paste0(filepath, "plot_phq_hard_cd.rds")
)
...
ggsave(
  filename = paste0(filepath_save, "Kaplan Meier Hard CD.pdf"),
  plot = plot,
  width = 9.5,
  height = 8,
  units = 'in'
