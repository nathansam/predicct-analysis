...
legend.title = 'Often feel fatigued'
...
readr::write_rds(
  x = plot_soft_uc,
  file = paste0(filepath_save, "plot_fatigue_soft_uc.rds")
)

# Soft cd
readr::write_rds(
  x = plot_soft_cd,
  file = paste0(filepath_save, "plot_fatigue_soft_cd.rds")
)

# Hard uc
readr::write_rds(
  x = plot_hard_uc,
  file = paste0(filepath_save, "plot_fatigue_hard_uc.rds")
)

# Hard cd
readr::write_rds(
  x = plot_hard_cd,
  file = paste0(filepath_save, "plot_fatigue_hard_cd.rds")
)
...
ggsave(
  filename = paste0(filepath_save, "Kaplan Meier fatigue.pdf"),
  plot = plot,
  width = 9,
  height = 9,
  units = 'in'
