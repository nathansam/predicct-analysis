...
### Somatisation grouped

```{r}
# Combine categories
data_survival_soft %<>%
  mutate(somatisation = case_when(
    somatisation %in% c("Moderate", "Severe") ~ "Moderate",
    TRUE ~ somatisation
  )) %>%
  dplyr::mutate(somatisation = forcats::as_factor(somatisation)) %>%
  dplyr::mutate(somatisation = forcats::fct_relevel(somatisation, "None", "Mild", "Moderate"))
