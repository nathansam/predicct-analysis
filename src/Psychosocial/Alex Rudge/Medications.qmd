...
# Dependent variable is DifficultyRemembering

# Statistical tests for each independent variable with DifficultyRemembering
# With p-values adjusted for multiple testing using holm correction.
statistical_tests <- summon_statistical_test(
  data = data_baseline, 
  dependent = 'DifficultyRemembering', 
...
# Kaplan Meier
data_survival_soft %>%
  survfit(Surv(time, DiseaseFlareYN) ~ DifficultyRemembering, data = .) %>%
  ggsurvplot(
    ., 
    data = data_survival_soft, 
    conf.int = TRUE, 
