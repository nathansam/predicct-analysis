...
data_vis_missingness <- data_hads %>%
  # Select anxiety hads
...
# FC, Smoke and IMD are often missing together
...
data_baseline_anxiety_uc <- data_hads %>%
  dplyr::filter(hads_type == 'anxiety_hads') %>%
...
data_baseline_anxiety_cd <- data_hads %>%
  dplyr::filter(hads_type == 'anxiety_hads') %>%
...
data_baseline_depression_uc <- data_hads %>%
  dplyr::filter(hads_type == 'depression_hads') %>%
  dplyr::filter(diagnosis2 == 'UC/IBDU')
...
data_baseline_depression_cd <- data_hads %>%
  dplyr::filter(hads_type == 'depression_hads') %>%
  dplyr::filter(diagnosis2 == 'CD')
...
# Patient reported
data_survival_soft <- data_hads %>% dplyr::inner_join(
...
# Hard flares
data_survival_hard <- data_hads %>% dplyr::inner_join(
