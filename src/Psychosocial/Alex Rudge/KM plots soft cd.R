...
plot_anxiety <- readr::read_rds(
  file = paste0(filepath, "plot_anxiety_soft_cd.rds")
)
...
plot_depression <- readr::read_rds(
  file = paste0(filepath, "plot_depression_soft_cd.rds")
)
...
plot_psqi <- readr::read_rds(
  file = paste0(filepath, "plot_psqi_soft_cd.rds")
)
...
plot_phq <- readr::read_rds(
  file = paste0(filepath, "plot_phq_soft_cd.rds")
)
...
ggsave(
  filename = paste0(filepath_save, "Kaplan Meier Soft CD.pdf"),
  plot = plot,
  width = 9.5,
  height = 8,
  units = 'in'
