...
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + AgeGroup + Sex + IMD + frailty(SiteNo), data = psqi_soft)
summary(cox_model)

#with FC
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + Sex + AgeGroup + cat + IMD +  frailty(SiteNo), data = psqi_soft)
summary(cox_model)
...
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + AgeGroup + Sex + IMD + frailty(SiteNo), data = psqi_hard)
summary(cox_model)

#with FC
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + Sex + AgeGroup + IMD + cat + frailty(SiteNo), data = psqi_hard)
summary(cox_model)
```
...
#no effect when controlling for Sex, frailty +/- FC / age
#?need to analyse age groups separately? Find out if the effect exists for specific groups only
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + AgeGroup + IMD + Sex + frailty(SiteNo), data = psqi_CD_soft)
summary(cox_model)

#with FC
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + IMD + Sex + AgeGroup + cat + frailty(SiteNo), data = psqi_CD_soft)
