...
Global PSQI score: sum of seven component scores (of note, questions answered by
a partner are not calculated in the score).

Patients were grouped by presence/absence of sleep disturbance using a global
PQSI cut off of 5, as score > 5 has a diagnostic sensitivity of 89.6% and
specificity of 86.5% in distinguishing good and poor sleepers. 



Notes on Methods:

Component 1: Subjective sleep quality—question 9 (39.6 in PREdiCCt) -
"SleepQuality" (Score 0-3)
...
Component 5: Sleep disturbance - sum question 5b to 5j ("WakeUpMiddleNight" to
...
Component 7: Daytime dysfunction (questions 7 and 8 = 39.8 and 39.9,
"StayingAwake" and "EnoughEnthusiasm")
Both scores 0-3, sum/2 and rounded up

``` {r, tidy psqi}
#53 missing ParticipantId (blank rows - exclude these)
#psqi %>% filter(is.na(ParticipantId)) %>% nrow()
...
We investigated for associations between presence of sleep disturbance (global
PSQI >5) and diagnosis type, sex, flares in last year, FC and age using
chi-square test.

Analyses were carried out for the IBD cohort as a whole, and for CD and UC/IBDU
separately.
...
* Sex: females have a statistically increased risk of sleep disturbance
  (p-value < 0.00001 )
* IBD type: There is no significant association between IBD type and sleep
  disturbance (p-value = 0.16)
* Flares in last year: is a significant association between flares in last year
  and presence of sleep disturbance (p-value = 0.001)
* FC category (<50, 20-250, >250): no statistically significant difference
  (p-value = 0.43)
* Age: Patients at extremes of age have higher rates of sleep disturbance
  (p-value > 0.00001)
...
if (chi_squared_result$p.value < alpha) {
  cat("There is a significant association between sex and presence of",
      "sleep disturbance.\n")
} else {
  cat("There is no significant association between sex and sleep disturbance.\n")
}
...
```{R}
#| label: fig-PSQI-dist
#| fig-cap: "Distribution of baseline PSQI Scores"
psqi2_percent <- psqi2 %>%
...
alpha <- 0.05
if (chi_squared_result$p.value < alpha) {
   cat("There is a significant association between IBD type and presence of",
       " sleep disturbance.\n")
} else {
  cat("There is no significant association between IBD type and sleep",
          " disturbance.\n")
}
pander(chi_squared_result)
```
...
alpha <- 0.05
if (chi_squared_result$p.value < alpha) {
  cat("There is a significant association between flares in last year and",
      "presence of sleep disturbance.\n")
} else {
  cat("There is no significant association between flares in last year and",
      "sleep disturbance.\n")
}
pander(chi_squared_result)
```
...
```{R}
#| label: fig-psqi-by-flare-dist
#| fig-cap: "Distribution of baseline PSQI Scores, divided by Flares in previous year"
...
# Plot the line graph
ggplot(psqi2_percent,
       aes(x = TotalScore, y = Percentage, colour = flare_group)) +
  geom_line() +
  labs(x = "PSQI score",
       y = "Percentage of Participants") +
  theme_classic()
...
```{R}
#| label: fig-sleep-disturbance-perc
#| fig-cap: "Percentage of Respondents with Sleep Disturbance across FC categories"
...
alpha <- 0.05
if (chi_squared_result$p.value < alpha) {
  cat("There is a significant association between FC and presence of sleep",
      "disturbance.\n")
} else {
  cat("There is no significant association between FC and sleep",
      "disturbance.\n")
}
pander(chi_squared_result)
```
...
alpha <- 0.05
if (chi_squared_result$p.value < alpha) {
  cat("There is a significant association between age and presence of sleep",
  "disturbance.\n")
} else {
  cat("There is no significant association between age and sleep disturbance.")
}
pander(chi_squared_result)
```
...
```{R}
#| label: fig-psqi-age
#| fig-cap: "Sleep Distrubance (PSQI >5) across age groups. Older patients have higher rates of sleep disturbnace, p-value > 0.00001"
percentages <- psqi2 %>%
...
Here we present the total PSQI scores by sex, age and UC/IBDU vs CD; as well as
the prevalence of sleep disturbance. 

```{r, table for sleep disturbance ALL}
#| label: tbl-sleep-cat
#| tbl-cap: "Sleep Disturbance Across Categories"
...
* Sex: females have a statistically increased risk of sleep disturbance
  (p-value = 0.00002 )
* Flares in last year: there is a significant association between flares in last
  year and presence of sleep disturbance (p-value = 0.0005)
* FC category (<50, 20-250, >250): no statistically significant difference
  (p-value = 0.200) 
* Age: Patients at extremes of age have higher rates of sleep disturbance
  (p-value = 0.0002) (highest in 55-64 age group)


```{r, table for sleep disturbance CD}
#| label: tbl-sleep-cat_cd
#| tbl-cap: "Sleep Disturbance Across Categories (CD)"
my.render.cont <- function(x) {
  with(
    stats.apply.rounding(stats.default(x),
      digits = 3,
...
table1::table1(~ Sex + AgeGroup + flare_group + cat | SleepDisturbance,
  data = psqi_CD,
  render.continuous = my.render.cont
)
...
alpha <- 0.05
if (chi_squared_result$p.value < alpha) {
  cat("There is a significant association between sex and presence of sleep",
      "disturbance.\n")
} else {
  cat("There is no significant association between sex and sleep",
      "disturbance.\n")
}
pander(chi_squared_result)
```
...
if (chi_squared_result$p.value < alpha) {
  ...
  cat("There is no significant association between flares in last year",
...
if (chi_squared_result$p.value < alpha) {
  ...
  cat("There is no significant association between FC and sleep disturbance.\n")
}
pander(chi_squared_result)
# Check if the result is significant at a certain alpha level (e.g., 0.05)
```
...
if (chi_squared_result$p.value < alpha) {
  ...
  cat("There is no significant association between age and sleep",
      "disturbance.\n")
}
pander(chi_squared_result)
```
...
```{R}
#| label: fig-psqi-age-cd
#| fig-cap: "Sleep Distrubance (PSQI >5) across age groups in CD. Patients at extremes of age have higher rates of sleep disturbnace, p-value > 0.0002."
percentages <- psqi_CD %>%
  group_by(AgeGroup, SleepDisturbance) %>%
  summarise(count = n()) %>%
  group_by(AgeGroup) %>%
...
ggplot(percentages,
       aes(x = AgeGroup,
           y = percentage,
           color = SleepDisturbance,
           group = SleepDisturbance)) +
  geom_line() +
  labs(x = "Age Groups",
       y = "Percentage of Paricipants") +
...
```{r, table for sleep disturbance CD}
#| label: tbl-sleep-cat_uc
#| tbl-cap: "Sleep Disturbance Across Categories (UC/IBDU)"
my.render.cont <- function(x) {
  with(
    stats.apply.rounding(stats.default(x),
      digits = 3,
...
```{R}
#| label: fig-psqi-age-uc
#| fig-cap: "Sleep Distrubance (PSQI >5) across age groups in UC/IBDU. Patients at extremes of age have higher rates of sleep disturbnace, p-value > 0.00001."
percentages <- psqi_UC %>%
  group_by(AgeGroup, SleepDisturbance) %>%
  summarise(count = n()) %>%
  group_by(AgeGroup) %>%
...
Whole cohort time to patient-reported flare

* Significant controlling for site only (p-value = 0.000049, 95% CI 1.20-1.68)
* Significant controlling for site, sex, smoking, IMD, Age group (p-value =
  0.001, 95% CI 1.12-1.58)
* Significant when controlling for FC additionally (p-value 0.0008, 95%
  CI 1.12-1.59)
...
test_result <- survdiff(Surv(time, DiseaseFlareYN) ~ SleepDisturbance,
                        data = psqi_soft)
pander(test_result)
```
...
```{R}
#| label: fig-soft-all
#| fig-cap: "Time to patient-reported flare by presence of sleep disturbance (all participants)."
p1 <- ggsurvplot(fit,
           data = psqi_soft,
           conf.int = TRUE,
...
#| output: "asis"
#site only 
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~
...
```{R}
#| output: "asis"
#no effect when controlling for Sex, frailty , age, IMD
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~
                     SleepDisturbance + AgeGroup + Sex + IMD + frailty(SiteNo),
                   data = psqi_soft)
invisible(cox.summary(cox_model))
```
...
```{R}
#| label: fig-hard-all
#| fig-cap: "Time to hard flare by presence of Sleep Disturbance (all participants)"
...
```{R}
#| output: "asis"
#no effect when controlling for Sex, frailty +/- FC / age
...
* Significant increase in risk in time to patient-reported flare controlling for site only
  (p-value = 7.4e-06, 95% CI 1.37-2.24)
* Significant when controlling for site, sex, smoking, IMD, age (p-value =
  0.0004, 95% CI 1.22 - 2.04)
* Significant when additionally controlling for FC (p-value = 0.0006,
  95% CI 1.12-2.02)
...
* No significant increase in risk of time to flare in any analyses
...
```{R}
#| label: fig-soft-uc
#| fig-cap: "Time to patient-reported flare by presence of Sleep Disturbance (CD patients)"
fit <- survfit(Surv(time, DiseaseFlareYN) ~ SleepDisturbance,
               data = psqi_CD_soft)
p.cd.sleep.clin <- ggsurvplot(fit,
           data = psqi_CD_soft,
           conf.int = TRUE,
           ggtheme = theme_minimal(),
           palette = c("#42213D", "#F51AA4"),
           xlab = "Time from study recrutiment (days)",
           title = "Time to patient-reported flare",
           legend.title = "Sleep distubance",
           legend.labs = c("PSQI ≤ 5", "PSQI > 5")
           )
p.cd.sleep.clin
```
...
fit.me <- coxph(Surv(time, DiseaseFlareYN) ~
                  Sex + AgeGroup + SleepDisturbance + IMD + cat + frailty(SiteNo),
                data = psqi_CD_soft)
...
#?need to analyse age groups separately? Find out if the effect exists for specific groups only
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~
                     SleepDisturbance + IMD + AgeGroup + Sex + frailty(SiteNo),
                   data = psqi_CD_hard)
invisible(cox.summary(cox_model))
```
...
#with FC
fit.me <- coxph(Surv(time, DiseaseFlareYN) ~
                  Sex + AgeGroup + SleepDisturbance + cat + IMD + frailty(SiteNo),
                data = psqi_CD_hard)
invisible(cox.summary(fit.me))
```
...
* no significant increase in risk of time to patient-reported flare in any analyses
...
p1 <- ggsurvplot(fit,
           ...
           conf.int = TRUE,
           ggtheme = theme_minimal(),
           legend.title = "UC patients, time to patient-reported flare by presence of Sleep Disturbance",
           palette = c("#42213D", "#F51AA4"),
           xlab = "Time from study recrutiment (days)",
           title = "Time to patient-reported flare"
           )
...
```{R}
#| output: "asis"
#no effect when controlling for Sex, frailty +/- FC
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~
                     SleepDisturbance + Sex + AgeGroup + IMD + frailty(SiteNo),
                   data = psqi_UC_soft)
invisible(cox.summary(cox_model))
```
...
p1 <- ggsurvplot(fit,
           ...
           conf.int = TRUE,
           ggtheme = theme_minimal(),
           legend.title = paste("UC patients, time to hard flare by presence",
...
#no effect when controlling for Sex, frailty +/- FC
fit.me <- coxph(Surv(time, DiseaseFlareYN) ~
                  Sex + AgeGroup + SleepDisturbance + IMD + frailty(SiteNo),
                data = psqi_UC_hard)
invisible(cox.summary(fit.me))
```
...
#| output: "asis"
fit.me <- coxph(Surv(time, DiseaseFlareYN) ~
                  Sex + SleepDisturbance + AgeGroup + IMD + cat + frailty(SiteNo),
                data = psqi_UC_hard)
invisible(cox.summary(fit.me))
```
