...
```{R}
#| label: fig-soft-all
#| fig-cap: "Time to soft flare by presence of sleep disturbance (all participants)."
p1 <- ggsurvplot(fit,
           data = psqi_soft,
           conf.int = TRUE,
...
* Significant increase in risk in time to soft flare controlling for site only
...
```{R}
#| label: fig-soft-uc
#| fig-cap: "Time to soft flare by presence of Sleep Disturbance (CD patients)"
...
#?need to analyse age groups separately? Find out if the effect exists for specific groups only
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + IMD + AgeGroup + Sex + frailty(SiteNo), data = psqi_CD_hard)
summary(cox_model)
```

```{R}
#with FC
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + Sex + IMD + AgeGroup + cat + frailty(SiteNo), data = psqi_CD_hard)
...
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + Sex + AgeGroup + IMD + frailty(SiteNo), data = psqi_UC_soft)
summary(cox_model)

cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + Sex + IMD + AgeGroup + cat + frailty(SiteNo), data = psqi_UC_soft)
...
#no effect when controlling for Sex, frailty +/- FC
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + Sex + AgeGroup + IMD + frailty(SiteNo), data = psqi_UC_hard)
...
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ SleepDisturbance + Sex + IMD + AgeGroup + cat + frailty(SiteNo), data = psqi_UC_hard)
