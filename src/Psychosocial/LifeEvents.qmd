...
Answer options were multiple choice and included a free text option 'other'. The options provided are a modified Holmes-Rahe Stress inventory.
...
#counts of events
lifeevents$NumberEvents <- rowSums(!is.na(lifeevents[, 8:25]), na.rm = TRUE)
...
LE_UC <- lifeevents[lifeevents$diagnosis2 == "UC/IBDU",]
LE_CD <- lifeevents[lifeevents$diagnosis2 == "CD",]
...
LE_UC_soft <- lifeevents_soft[lifeevents_soft$diagnosis2 == "UC/IBDU",]
LE_CD_soft <- lifeevents_soft[lifeevents_soft$diagnosis2 == "CD",]

LE_UC_hard <- lifeevents_hard[lifeevents_hard$diagnosis2 == "UC/IBDU",]
LE_CD_hard <- lifeevents_hard[lifeevents_hard$diagnosis2 == "CD",]
```


#### Whole cohort 

Soft flaree (n= 1596, number of events= 579) - no significance across any analysis.
Hard flare (n= 1552, number of events= 215) - no significance across any analysis.
...
p1 <- ggsurvplot(fit,
           ...
           legend.title = "All patients, time to soft flare by presence of signficant life event",
...
Soft flare (n= 782, number of events= 305) - no associations. 
Hard flare (n= 763, number of events= 118) - no associations.
...
p1 <- ggsurvplot(fit,
           ...
           conf.int = TRUE,
           ggtheme = theme_minimal(),
           legend.title = "UC/IBDU patients, time to soft flare by presence of signficant life event",
           palette = c("#42213D", "#F51AA4"),
           xlab = "Time from study recrutiment (days)",
           title = "Time to soft flare"
           )
...
Soft flare (n= 814, number of events= 274) - no significance across any analysis.
Hard flare (n= 789, number of events= 97) - no significance across any analysis.
...
p1 <- ggsurvplot(fit,
           ...
           conf.int = TRUE,
           ggtheme = theme_minimal(),
           legend.title = "CD patients, time to soft flare by presence of signficant life event",
           palette = c("#42213D", "#F51AA4"),
           xlab = "Time from study recrutiment (days)",
           title = "Time to soft flare"
           )
...
#no effect when controlling for Sex, Smoke, frailty , age, IMD
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ AnyLifeEvents + AgeGroup + Sex + Smoke + IMD + frailty(SiteNo), data = LE_CD_soft)
summary(cox_model)

#with FC
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ AnyLifeEvents + Sex + AgeGroup + Smoke + cat + IMD +  frailty(SiteNo), data = LE_CD_soft)
summary(cox_model)
...
#no effect when controlling for Sex, Smoke, frailty , age, IMD
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ AnyLifeEvents + AgeGroup + Sex + Smoke + IMD + frailty(SiteNo), data = LE_CD_hard)
summary(cox_model)

#with FC
cox_model <- coxph(Surv(time, DiseaseFlareYN) ~ AnyLifeEvents + Sex + AgeGroup + Smoke + cat + IMD +  frailty(SiteNo), data = LE_CD_hard)
summary(cox_model)
```