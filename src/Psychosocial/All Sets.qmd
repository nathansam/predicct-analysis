...
softflareplot <- readRDS("/Volumes/igmm/cvallejo-predicct/people/chiara/softflareplot.RDS")
hardflareplot <- readRDS("/Volumes/igmm/cvallejo-predicct/people/chiara/hardflareplot.RDS")
...
ggplot(hardflareplot, aes(x = Variable, y = HR, ymin = Lower95, ymax = Upper95, shape = Diagnosis, color = Variable)) +
  ...
  guides(color = guide_legend(override.aes = list(shape = 15, linetype = 0)),
       shape = guide_legend(override.aes = list(linetype = 0), reverse = TRUE)) +
  theme_minimal()


ggplot(softflareplot, aes(x = Variable, y = HR, ymin = Lower95, ymax = Upper95, shape = Diagnosis, color = Variable)) +
  geom_pointrange(position = dodge, size = 0.8) +  # For the point estimate and CI range
  geom_hline(yintercept = 1, linetype = 2) +  # Adds a reference line at y = 1 (null effect)
  coord_flip() +  # Flip coordinates for a horizontal forest plot
