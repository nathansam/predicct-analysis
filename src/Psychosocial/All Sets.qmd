...
exercise <- readRDS(paste0(save_path, "exercise.RDS"))
psqi_IBD <- readRDS(paste0(save_path, "psqi_IBD.RDS"))
phq_clean <- readRDS(paste0(save_path, "phq_clean.RDS"))
hads_fc <-  readRDS(paste0(save_path, "hads_fc.RDS"))
ETOH <- readRDS(paste0(save_path, "ETOH.RDS"))
hads_with_demographics <- readRDS(paste0(save_path, "hads_with_demographics.RDS"))
demo_uc <- readRDS(paste0(outdir, "demo-uc.RDS"))
demo_cd <- readRDS(paste0(outdir, "demo-cd.RDS"))
softflareplot <- readRDS(paste0(save_path, "softflare_plot.RDS"))
hardflareplot <- readRDS(paste0(save_path, "hardflare_plot.RDS"))
# if needed
...
p2 <- ggplot(hardflareplot, aes(x = Variable, y = HR, ymin = Lower95, ymax = Upper95, shape = Diagnosis, color = color_group)) +
  geom_pointrange(position = dodge, size = 0.7) +  # For the point estimate and CI range
  geom_hline(yintercept = 1, linetype = 2) +  # Adds a reference line at y = 1 (null effect)
  coord_flip() +  # Flip coordinates for a horizontal forest plot
...
# Cause of patient reported flares

# Select the columns of interest
flarecause <- flarecause %>%
  filter(!is.na(ParticipantId))

variables_of_interest <- flarecause %>%
  dplyr::select(c(
...
variables_count <- variables_of_interest %>%
  tidyr::pivot_longer(
...
# Joining variables (common to all dataframes)
joining_variables <- c('ParticipantNo', 'diagnosis2', 'age', 'AgeGroup', 'Sex', 'cat', 'IMD', 'SiteNo')

#inclusive full dataset 1639
all_psychosocial <- psqi_IBD %>% 
  full_join(
    exercise %>% 
      select(tidyselect::all_of(joining_variables), MinimumExercise),
...
all_psychosocial <- all_psychosocial %>% 
  full_join(
    hads_with_demographics %>% 
      select(tidyselect::all_of(joining_variables), anxiety_hads, depression_hads),
    by = joining_variables
  )
...
all_psychosocial <- all_psychosocial %>% 
  full_join(
    phq_clean %>% 
      select(tidyselect::all_of(joining_variables), TotalPHQ, somatisation),
    by = joining_variables
  )
...
all_psychosocial <- all_psychosocial %>% 
  full_join(
    ETOH %>% 
      select(tidyselect::all_of(joining_variables), Alcoholg, Alcohol_units_wk, weekly_units),
    by = joining_variables
  )
...
cd_psychosocial <- cd_psychosocial %>% 
  left_join(
    demo_cd %>% select(ParticipantNo, Location, L4, Behaviour, Perianal, HBI, Surgery, Smoke),  
    by = "ParticipantNo"
    )
...
uc_psychosocial <- uc_psychosocial %>% 
  left_join(
    demo_uc %>% select(ParticipantNo, Extent, Mayo, Smoke), 
    by = "ParticipantNo"
    )
...
demo_IBD %>%
  dplyr::bind_rows(
    demo_IBD %>% dplyr::filter(ParticipantNo %in% all_psychosocial$ParticipantNo),
    .id = 'Group'
  ) %>% 
  dplyr::mutate(
...
# all_psychosocial %>%filter(is.na(Sex)) %>% nrow()
# all_psychosocial %>%filter(is.na(cat)) %>% nrow()
# all_psychosocial %>%filter(is.na(SiteNo)) %>% nrow()
# all_psychosocial %>%filter(is.na(IMD)) %>% nrow()
...
#wilcox.test(flares_last_year ~ Sex, data = hads_old_flare)
