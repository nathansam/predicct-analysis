...
all_psychosocial <- psqi_IBD %>% 
  full_join(exercise %>% select(ParticipantNo, MinimumExercise, diagnosis2, AgeGroup, cat, IMD, SiteNo))

all_psychosocial <- all_psychosocial %>% 
  full_join(hads_with_demographics %>% select(ParticipantNo, anxiety_hads, depression_hads, diagnosis2, cat, AgeGroup, IMD, SiteNo))

all_psychosocial <- all_psychosocial %>% 
  full_join(phq_clean %>% select(ParticipantNo, TotalPHQ, somatisation, diagnosis2, AgeGroup, cat, IMD, SiteNo))

all_psychosocial <- all_psychosocial %>% 
  full_join(ETOH %>% select(ParticipantNo, Alcoholg, Alcohol_units_wk, weekly_units, diagnosis2, AgeGroup, cat, IMD, SiteNo))
...
missing_soft <- all_soft %>% 
  filter(is.na(IMD))
#no missingL DiseaseFlareYN, time, cat, SiteNo (fixed)
#17 missing IMD (all in exercise)

missing_allsets <- all_psychosocial %>% 
  filter(is.na())
#calculate total soft flare
